"""Smoke-run the Drop Jump pipeline on a real video.

Usage: uv run scripts/smoke_drop_jump.py [video_path]
"""
...
from src.tests.physical.drop_jump import DropJumpTest  # noqa: E402

DEFAULT_VIDEO = (
    ROOT / "data" / "01. Physical Capabilities" / "Drop-Jump"
    / "VID_20251118_133028764 (1).mp4"
)
...
def main() -> int:
    ...
    out_dir = ROOT / "outputs" / "smoke_drop_jump"
    out_dir.mkdir(parents=True, exist_ok=True)
    athlete = AthleteProfile(gender="M", age=20)

    print(f"video      : {video}")
    print(f"output_dir : {out_dir}")
    print("loading model + running...")
    ...
    for mid, mv in result.metrics.items():
        ...
        if s is None:
            print(f"  {mid:<24} {mv.raw:.3f} {mv.unit}  [no benchmark]")
        else:
            print(
                f"  {mid:<24} {mv.raw:.3f} {mv.unit}  "
