...
Horizontal jump distance from standstill. Lower-body horizontal power.

## 2. Equipment & setup

Marked landing zone with distance markings. Camera side-on, perpendicular to jump direction.

## 3. Protocol

1. Athlete stands behind take-off line
2. Countermovement allowed (arm swing, knee bend)
3. Jumps horizontally as far as possible
4. Lands on both feet, distance measured to closest body part to line
...
| Cone detection | no | |
| Calibration | mandatory (distance markings) | |
| Event detection | — | takeoff, landing (closest point) |

## 5. Metrics produced

See `docs/metrics/METRICS_CATALOG.md` for formulas and units.

**Jump:** `jump_distance_cm` (primary), `peak_height_cm` (max vertical of COM during flight), `flight_time_s`, `peak_takeoff_acceleration_ms2`
...
- HUD ticker fields: `jump_distance_live`
- End-card: all metrics + test score

## 7. Benchmark schema

- File: `benchmarks/physical/standing-long-jump.yaml`
...
ACSM standard. Calibration via floor markings is critical.
